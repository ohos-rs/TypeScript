...
export function getNodeCommonData(node: Node): number {
    switch (node.kind) {
        ...
        case SyntaxKind.ImportClause:
            return ((node as ImportClause).isLazy ? 1 : 0) << 24 | ((node as ImportClause).phaseModifier === SyntaxKind.TypeKeyword ? 1 : (node as ImportClause).phaseModifier === SyntaxKind.DeferKeyword ? 2 : 0) << 25;
